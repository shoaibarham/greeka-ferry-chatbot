...
# Import models after app is created (but before routes that use models)
...
try:
    ...
    logger.warning("FerryAgent could not be imported. Chat functionality will be limited.")
...
def chat():
    ...
    if ferry_agent is None:
        initialize_agent()
        if ferry_agent is None:
            return jsonify({"error": "Ferry agent is not available. The system may not be configured with proper API keys."})
